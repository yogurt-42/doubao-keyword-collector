...
- P0/P1 阶段修复、平台类型改造、长尾信源分析、定时任务、Web UI 与 Native 对齐、URL 匹配性能优化、账号标签显示/隐藏切换、验证码人机验证处理、Phase 3 性能优化专项、应用内检查更新、应用内下载更新（双版本下载 + SHA256/兜底校验 + 本地缓存）、豆包新布局兼容均已完成。当前下一阶段为“应用内自动更新”的剩余部分：Windows 自替换 installer，详见 `ROADMAP.md`。
